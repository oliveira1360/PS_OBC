...
void i2c_tick(i2c_handle_t *h)
{
    ...
    case I2C_STARTING:
        ...
        if (h->use_reg)
        {
            /* Primeiro envia endereço com W para escrever o registo */
    ...
    case I2C_RESTART:
        /* Espera que o byte do registo seja enviado */
        if (!hal_i2c_tx_ready())
        {
        ...
        /* Gera Repeated START com endereço + R */
        hal_i2c_restart_read(h->addr);
        h->use_reg = 0;  /* já enviámos o registo, agora é leitura normal */
        h->state = I2C_WAIT_RESTART;
        break;

    case I2C_WAIT_RESTART:
        /* Espera que o restart complete e verifica ACK */
        if (!hal_i2c_get_ack())
        {
            hal_i2c_stop();
            h->state = I2C_IDLE;
