...
qspi_boot_result_t qspi_boot_wren(void)
{
    ...
    while (retries > 0U)
    {
        ...
        while (!(QSPI_SR & QSPI_SR_INSTRE))
        {
            if (--timeout == 0U) return QSPI_BOOT_TIMEOUT_ERR;
        }
        (void)res;
...
qspi_boot_result_t qspi_boot_wait_busy(void)
{
    ...
    while (timeout > 0U)
    {
        ...
        while (!(QSPI_SR & QSPI_SR_INSTRE))
        {
            if (--inner == 0U) return QSPI_BOOT_TIMEOUT_ERR;
        }
...
qspi_boot_result_t qspi_boot_erase_sector(uint32_t addr)
{
    ...
    /* 1. Write Enable */
    res = qspi_boot_wren();
    if (res != QSPI_BOOT_OK) return res;
                                          ...
                                          uint32_t len)
{
    ...
    if (len == 0U || len > W25Q_PAGE_SIZE) return QSPI_BOOT_TIMEOUT_ERR;
